...
class TCNBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.conv(x)
        out = out[..., :x.shape[2]]  # crop to input length
        out = self.relu(out)
...
def plot_training_results(train_losses: List[float], val_losses: List[float], maes: List[float] = None, lrs: List[float] = None, early_stop_epoch: int = None):
    ...
    plt.title('Процесс обучения TCN')
    plt.xlabel('Эпоха')
    plt.ylabel('Ошибка')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'tcn_training_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
    plt.close()
    # Сохраняем данные для анализа
    df = pd.DataFrame({'train_loss': train_losses, 'val_loss': val_losses if val_losses else [None]*len(train_losses), 'mae': maes if maes else [None]*len(train_losses), 'lr': lrs if lrs else [None]*len(train_losses)})
    df.to_csv(f'tcn_training_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', index=False)
...
def main():
    ...
    tcn = TCN(input_size=5, num_channels=NUM_CHANNELS).to(device)
    print("Обучение TCN...")
    tcn_losses, tcn_val_losses = train_model(
        tcn,
        train_loader,
        val_loader,
        NUM_EPOCHS,
    ...
    plot_training_results(tcn_losses, tcn_val_losses)
    scaler_params = {
        'feature_scaler': {
    ...
    with open('tcn_scaler_params.json', 'w') as f:
        json.dump(scaler_params, f)
    print('Обучение TCN завершено. Модель и параметры сохранены.')
